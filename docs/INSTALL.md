...
(`scripts\run-web.ps1`, which the double-clickable root `run-web.bat` calls â€” the
...
- **macOS / Linux**: a POSIX shell (the system `sh` is enough) â€” nothing else
...
Once the pack is installed, start the app with the run launcher â€” it *is* the
...
2. streams the app's own output to the terminal â€” nothing is swallowed â€” and
...
3. opens **that** URL, token included, in **Google Chrome** â€” found on `PATH`,
...
- **The launch token is a live credential.** It is the value the server
  ...
  **in memory only**: the launchers never write it to a file, never echo it
  themselves, and never build a command string out of it â€” it reaches the browser
...
`uninstall.bat` (Windows) or `./uninstall.sh` (macOS/Linux) â€” removes the bundles
...
1. Start (or restart) the app with **`run-web.bat`** (Windows) or **`./run-web.sh`**
   (macOS/Linux) â€” or `npx @deepseek-ai/dsh web` by hand â€” and open/select a
   conversation.
2. Open the right Sidebar with the **expand button** in the conversation header
   ...
   Files tab, the new **Editor** and **History** â€” the workspaceâ€™s commit
   history â€” it needs `git` on the `PATH` of the host running `dsh web`). The tab
   stripâ€™s **"+"** opens that Start page again at any time.
3. The headerâ€™s **Terminal** button (the last one in that group, right of
   **Open Inâ€¦**) opens the **bottom dock**: a real shell in the conversationâ€™s
   ...
   interrupts). It closes with the same button or its `Ã—`; the shell is kept
   briefly, so reopening the dock in that conversation reattaches to it. The
   dock's left edge follows the left bar when that is collapsed or expanded â€”
...
4. The left bar's top row wears the pack's branding: the **app icon** (a black
   ...
   wordmark were â€” and the same icon replaces the whale beside **Into the
   Unknown** on the new-session screen.

## Troubleshooting

- **The run launcher says the port is already in use** â€” an app is already
...
- **`run-web.bat` refuses to start / the window flashes and closes** â€” the wrapper
...
- **No browser opened, but the app is running** â€” the ready line was never
...
- **The browser opened but Chrome is not installed** â€” that is the fallback
  working: the launcher reports "the default browser" instead of Chrome. Pass
  `-DefaultBrowser` to skip the Chrome lookup entirely.
- **The launcher refused to open the URL** â€” the ready line did not name a
...
- **`./run-web.sh: Permission denied`** â€” `chmod +x run-web.sh install.sh uninstall.sh
  scripts/*.sh`.
- **The launcher says the profile does not list this pack's bundles** â€” a
  friendly warning, not a failure: the app still starts. Run `install.bat` /
  `./install.sh` if you expected the pack in it.
- **`dsh` exits non-zero during install** â€” most often a network hiccup fetching
  the pinned CLI; re-run, and use `-Verbose` on the PowerShell half to see the
  exact command.
- **Profile not found** â€” pass `-DshHome`/`-ProfileName`, or run
  `npx @deepseek-ai/dsh web` once so the profile exists.
- **`./install.sh` says a command is missing** â€” install Node.js 22+
  (https://nodejs.org). The macOS/Linux half needs Node and npm/npx only; it
  never needs PowerShell.
- **`sh: scripts/install-all.sh: not found` (or a syntax error)** â€” run it from
...
- **`./install.sh: Permission denied`** â€” `chmod +x install.sh uninstall.sh
  scripts/*.sh`.
- **`-Target desktop` is rejected** â€” intentional: DSH Desktop is no longer a
  target of this pack; run without `-Target` (or with `-Target web`).
- **Two Files panels / a stray right-hand dock after upgrading** â€” the retired
  `dsh-files` (or `dsh-focus`) bundle is still in the profile; re-run the
  installer so its prune removes it.
- **No "Editor" capsule on the "+" / Start page** â€” pick an active
...
- **A text file opens in the read-only preview** â€” that extension belongs to a
  shipped preview (`.html`, images, `.pdf`, â€¦) or the path is outside the
...
- **Save-as says the name is taken / the folder is missing** â€” pick another name
  (`409 EXISTS`), or use a subfolder that already exists (`404 NO_FOLDER`):
  nothing creates directories.
- **The "Open Inâ€¦" File Explorer entry does nothing** â€” confirm the boot HTML
  lists `dsh-open-in-app/client.js` and not
  `@deepseek-ai/dsh-client-ui-open-in-app`; the pack's launcher reports a real
  failure (HTTP 502 â†’ the button's red state) instead of a silent success.
- **Editor says "Could not open the file" / keeps saving as changed on disk** â€”
...
- **No Themes button in the header (or it is greyed out)** â€” the button sits
  immediately left of **Open Inâ€¦**; a new package needs one install run
  ...
  in the boot graph â€” the tooltip says "The theme service is unavailable".
- **A header icon button has no circle around it** â€” the group's round hairline
  ...
  later.
- **Code text looks black-on-dark in the light theme** â€” the editor follows the
...
- **Blank lines in a Markdown file render as full empty lines, or the "Edit"
  button looks monospaced** â€” the preview's plain-text scrollport
...
- **"Preview" says it is unavailable** â€” the right bar's controller could not be
  ...
  which.
- **A Markdown tab still offers "Markdown" / "Plain text" in its header** â€” that
  viewer menu is hidden on Markdown tabs by `dsh-themes` alpha.3; reinstall so
  that version is in the profile, then restart.
- **The rendered Markdown page has no "Edit" button** â€” that button is the
  ...
  restart.
- **The rendered Markdown view is still dark, or unreadable on white** â€” that is
...
- **No "History" capsule on the "+" / Start page** â€” `dsh-gittree` is not mounted;
  ...
  does not show.
- **History sits on the "Reading the historyâ€¦" and never stops** â€” that was
  alpha.1â€™s bug (an effect cleanup cancelled the request it had just
...
- **History says "Not a git repository"** â€” the conversation folder is not inside
...
- **History says "git is not installed"** â€” `git` is missing from the `PATH` of
  the host running `dsh web` (the routes spawn it directly).
- **History opens a file in the wrong tab, or not at all** â€” the row hands the
...
- **No Terminal button in the header** â€” `dsh-terminal` is not mounted; a new
  ...
  right of **Open Inâ€¦**. Check the console for `[dsh-terminal]` if it still does
  not show.
- **The dock says "No terminal on this host"** â€” the harness installation's
...
- **The dock opens but the panel does not make room for itself** â€” something is
...
- **A shell's output is gone after a page reload** â€” the shell itself is kept for
...
- **The terminal shows the wrong number of lines after resizing, or the newest
  output is not visible** â€” that was alpha.1; alpha.2 re-fits the emulator on
...
- **Opening the dock moves the items in the left bar up** â€” that was alpha.1
...
- **The dock keeps the old left edge after collapsing or expanding the left
  bar** â€” that was alpha.2 (only the frame's `style` mutation was watched, and the
...
- **A terminal chip you just left keeps the selected dress, and terminals past the
  right edge of the bar cannot be reached** â€” both were alpha.3: the pick went
  ...
  makes the strip a scrolling box with the `+` beside it, `â€¹`/`â€º` arrows that
...
- **The left bar still shows the fish and the "deepseek" wordmark** â€” the branding
...
- **The mark looks clipped or oval** â€” alpha.7 drew it as a CSS disc, which lost a
...
- **The left bar's branding looks unstyled or empty after a harness update** â€”
...
- **No download icon in the header (the three-dot button is still there)** â€”
  ...
  (after **Open Inâ€¦**), so the old button disappearing IS the change.
- **The download icon is greyed out with "Session export is unavailable"** â€” the
...
- **`Ctrl+C` in the terminal copies instead of interrupting** â€” it must not: a
...
- **The terminal is a full shell with no sandbox** â€” that is what a terminal is.
