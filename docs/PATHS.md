...
2. **What is *not* in that directory is the problem.** Today the app's
   *runtime* — the harness itself, the packages it resolves, the pnpm that
   drives the profile — lives in the npm/npx cache and the pnpm store, both
   outside the home and outside the distribution folder. §3 lists those, and
   §5 states the rule that removes them.
...
root** — with the three exceptions in §3.
...
installed into the profile — they resolve through the installation fallback
(§2). This is the single most useful fact on this page for the offline goal:
...
heals on the next boot — and it takes a cross-process lock while doing it.
...
- **The installation is relocatable.** Wherever `@deepseek-ai/dsh` is run from
  *is* the installation; the junctions follow it. Vendoring the harness into the
  ...
  harness change required. (The harness also has a second mode —
...
- **Today that anchor points into the npm cache.** Delete or prune the cache
  ...
  with an intact-looking home. That is the concrete fragility §4 of the plan
  removes.
...
|---|---|---|---|---|
| **npm / npx cache** | `%LOCALAPPDATA%\npm-cache\_npx\<hash>` | `~/.npm/_npx/<hash>` | `npx @deepseek-ai/dsh@<pin>` unpacks the whole installation (measured: **223 MB, ~25 400 files**) here on first run | The profile's junctions point into it (§2). It is also outside the folder, so a distribution is not self-contained, and the first run needs the network. |
| **pnpm store** | `%LOCALAPPDATA%\pnpm\store\v3` | macOS `~/Library/pnpm/store`, Linux `~/.local/share/pnpm/store` (`$PNPM_HOME/store`, else `$XDG_DATA_HOME/pnpm/store`, win over these) | the store pnpm verified the profile against — the profile's own path is recorded in `profiles/web/node_modules/.modules.yaml` (`storeDir`) | Only needed to *install*; but the installer reads it, and the OS default differs, so a shipped store must pin `store-dir` itself. |
...
1. the **harness** runs from inside the folder, so `INSTALL_ANCHOR` — and with it
   every junction in `profiles/node_modules` — resolves inside the folder;
...
4. **no step reaches the network except a model call** — the pinned harness, its
...
§3's first three rows are exactly what stands between the current build and that
