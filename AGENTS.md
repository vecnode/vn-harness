...
- `packages/<bundle>/` - one standalone bundle per plugin, or a **parked** package (one that declares `dsh.bundlePending`, which the installers skip). Today:
  ...
  - `packages/dsh-browser/` - **PARKED: NOT DEPLOYED** (the package declares `dsh.bundlePending` where `dsh.bundle` goes, and both installers discover a bundle by `dsh.bundle` ALONE, so they skip it: no `dsh.profile.bundles` entry, no patch layer, nothing to install or uninstall - rename the key back and run the installer with `-Force` to ship it, and uninstall FIRST before parking it again, because the uninstaller finds packages the same way and cannot see a parked one). A BROWSER TAB on the bar, labelled **Browser**: a PAGE tab type (no `patterns`, `priority: 'builtin'`, address `sidebar://browser`) with one guide entry at `order: 40`, i.e. after Files (10), Editor (20) and History (30) on the Start page. The page is drawn by the browser already showing the GUI - ONE `<iframe>` - so the pack ships **no engine and downloads nothing**, and it never looks up an OS browser (a Chromium driven over CDP was built, measured and rejected by the owner: 115 MB fetched into `$DSH_HOME` for arbitrary-site control the iframe cannot offer). The consequence is designed around, not hidden: a cross-origin frame is OPAQUE, so in-frame navigation is invisible (back/forward replay the addresses THIS tab loaded), the LED reports only the one signal a frame gives (`load`: grey idle, amber loading, green loaded, red cannot-be-shown, 12 s of loading becomes an explicit note), and stop/reload may only unload and re-assign `src` because `contentWindow.location.reload()` is a cross-origin access that throws. Its ONE host surface is `GET /api/dsh-browser/probe?url=` (`connection.fetch`, `requestBody: 'buffered'`, Node's own TLS - never system tools): one ranged GET capped at 64 KiB gives both the framing verdict (`X-Frame-Options` / CSP `frame-ancestors`, with the CSP directive winning and a source's PATH deliberately ignored, so the verdict can only be too permissive) and the document `<title>`, which also resolves redirects for the address bar. ONLY `frameable === false` blocks a load; a 401/DNS/timeout/non-HTML answer is a `note` and the address goes to the frame anyway, because the frame uses the browser's own session and cookies while the probe is anonymous. The render surface is the SEAM: `SURFACES` maps a name to a factory returning `{ kind, mount, goto, reload, stop, dispose }` plus an `onState` reporter, and everything above it (toolbar, history stack, probe, LED, tab type, Start entry) is surface-neutral, so a harness-owned engine is one more entry and nothing else. No fork, no disabled core row, nothing for `sync-vendored.ps1` to track
...
# The token is a HANDSHAKE, not an API credential: `GET /?token=<t>` answers 303
# with the session cookie, and every /api/* route wants THAT cookie (a token
# query on an API path is a 401). A plain `fetch` does not keep cookies - capture
# `set-cookie` from the 303 and send it back, or the probe/route smoke silently
# checks nothing.
# It must also NOT list dsh-vn-master: the master is blank, with no client half.
# A PARKED package (dsh-browser) must not appear either: only a package with
# `dsh.bundle` is installed at all, so parking one is verifiable here.
