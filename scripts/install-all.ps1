...
Write-Host '    navigation, rotation, selectable text, find in document); it replaces the'
Write-Host '    shipped bare PDF view while every other file type keeps its surface.'
Write-Host '    Reading needs nothing installed; page pictures need poppler (pdftoppm),'
Write-Host '    mutool or Ghostscript on the server host, and the tool says so if none is'
Write-Host '    there. PDFs attached to the chat (under <DshHome>/attachments) are read by'
Write-Host '    their absolute path.'
