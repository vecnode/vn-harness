...
Write-Host '    navigation, rotation, selectable text, find in document); every page with'
...
Write-Host '    text. It replaces the shipped bare PDF view while every other file type'
Write-Host '    keeps its surface. Reading needs nothing installed; page pictures and'
Write-Host '    scanning need poppler (pdftoppm), mutool or Ghostscript on the server'
Write-Host '    host, and OCR needs tesseract with the language data - the tools say'
Write-Host '    which one is missing instead of failing. PDFs attached to the chat (under'
Write-Host '    <DshHome>/attachments) are read by their absolute path.'
