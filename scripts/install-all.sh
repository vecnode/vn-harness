...
printf '    navigation, rotation, selectable text, find in document); it replaces the\n'
printf '    shipped bare PDF view while every other file type keeps its surface.\n'
printf '    Reading needs nothing installed; page pictures need poppler (pdftoppm),\n'
printf '    mutool or Ghostscript on the server host, and the tool says so if none is\n'
printf '    there. PDFs attached to the chat (under <DshHome>/attachments) are read by\n'
printf '    their absolute path.\n'
