...
printf '    navigation, rotation, selectable text, find in document); every page with\n'
...
printf '    text. It replaces the shipped bare PDF view while every other file type\n'
printf '    keeps its surface. Reading needs nothing installed; page pictures and\n'
printf '    scanning need poppler (pdftoppm), mutool or Ghostscript on the server\n'
printf '    host, and OCR needs tesseract with the language data - the tools say\n'
printf '    which one is missing instead of failing. PDFs attached to the chat (under\n'
printf '    <DshHome>/attachments) are read by their absolute path.\n'
