...
- **Zero npm dependencies.** No shipped package declares a runtime dependency, so
  ...
  vendored files in this repo (Mermaid, CodeMirror, xterm.js) or resolved from
  the harness installation already on disk (`node-pty`, `ws`).
...
- **Vendored engines are pinned and hashed.** `dsh-editor`'s CodeMirror 6,
  ...
  `lib/vendor/VERSION.json`) and `dsh-terminal`'s `@xterm/xterm@5.5.0` +
  `@xterm/addon-fit@0.10.0` are built from their `vendor/` folders and served by
  the packages themselves — no CDN, no runtime download.
