...
It installs into exactly one place â€” the raw web profile
(`npx @deepseek-ai/dsh web`) â€” and everything it does is a file in this
...
- **No secrets in this repository.** API keys live in your own harness settings
  (`Settings â†’ Models`); no plugin or installer reads, writes, prompts for or
...
- **No core patching.** Every plugin is a standard dsh **bundle**
  ...
  hard-disabling the core rows through its own bundle layer â€” a supported patch
...
| Other published harness lines (`0.1.5-rc.2`, `0.1.5-rc.3`, `0.1.7-alpha.1`, â€¦) | **no** â€” not tested, and not claimed |
| Older pins / `master` APIs | no â€” upgrade the pin, then re-verify |
...
If you find a security issue in this pack â€” a plugin, the installers, the run
...
1. **Do not open a public issue with exploit details.** Open a private report
   instead: on GitHub, **Security â†’ Report a vulnerability** in
...
**What is worth protecting.** The app puts the user's own machine â€” the files
...
terminal dock â€” behind one browser session. That is the asset.
...
your unlocked session, and â€” most importantly â€” **deliberate exposure**. If you
...
1. **The bind.** The web composition binds `127.0.0.1` (the pack's launchers
   ...
   explicit message â€” `--host 0.0.0.0 is intentionally not supported yet for
   safety: it would expose remote code execution to the network` â€” so exposing
...
**1. Keep it on loopback â€” the default is already correct.**
...
there is no firewall rule that "fixes" an exposed bind â€” not exposing it is the
control.
...
a command string out of it â€” it reaches the browser as a single argument â€” but
...
| Start the app on a **different port** (`-Port 3091`) | the cookie's name and signed audience are derived from `host:port`, so old cookies no longer match â€” temporary, and they match again if you go back to the old port |
...
entry â€” the file also holds your model API keys, so do not delete the file â€” save
...
profile patch (`$DSH_HOME/profiles/web/cordis.patch.yml`) â€” a patch replaces the
...
you left open on an unlocked screen, and it does not sandbox the model's tools â€”
...
#    In the browser: DevTools -> Application -> Cookies -> the dsh-auth-â€¦ entry
...
How the pack handles it (`run-web.bat` on Windows, `run-web.sh` on macOS/Linux â€” the two
...
- it is **read in memory** from the app's own output and never written to a file
  ...
  exactly this reason);
- it is **never echoed by the launchers** â€” they only report the origin they
  opened â€” and it is never put in a URL logged anywhere else;
- it reaches the browser as **one argv element**, never through a command string:
  ...
  `scripts/run-web.ps1` â€” no `cmd /c start`, no `sh -c`, so nothing in it can be
...
revocation steps). If a token leaks, restart the app â€” that URL is dead. If a
...
- **Vendored engines are pinned and hashed.** `dsh-editor`'s CodeMirror 6,
  ...
  from their `vendor/` folders and served by the packages themselves â€” no CDN,
  no runtime download.
...
- **No way to weaken the harness's authentication.** The pack mounts every route
  ...
  pack adds capability *behind* that check â€” never a way around it.
...
- **Keep the app on loopback.** It is a personal tool with an unsandboxed
  terminal behind its authentication; binding it to a shared network interface â€”
  or running it on a machine other people can use â€” puts a shell behind a single
  cookie.
